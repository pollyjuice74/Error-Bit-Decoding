...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        captions = self.embedding(captions) # [N, T, W] <- [N, T]
        captions = self.positional_encoding(captions) # Add pos encoding

        features = self.visual_projection(features) # [N, W] <- [N, D]
        features = features.unsqueeze(1).repeat(1, T, 1) # [N, T, W] <- [N, 1, W] <- [N, W]

        tgt_mask = torch.tril(torch.ones(T,T)) # Triangular matrix of ones 

        out = self.transformer(tgt=captions, memory=features, tgt_mask=tgt_mask)
